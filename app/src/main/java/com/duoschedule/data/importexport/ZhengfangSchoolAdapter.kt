...
class ZhengfangSchoolAdapter(
    private val baseUrl: String = ""
) : SchoolAdapter {

    companion object {
        private const val TAG = "ZhengfangSchoolAdapter"
    }

    override val schoolInfo = SchoolInfo(
        id = "zhengfang_default",
        name = "正方教务系统（通用）",
        baseUrl = "",
        systemType = "zhengfang",
        loginUrl = if (baseUrl.isNotEmpty()) "${baseUrl.trimEnd('/')}/default2.aspx" else "",
        scheduleUrlPath = "/xskbcx.aspx"
    )
    ...
    override suspend fun login(
        ...
        try {
            val effectiveBaseUrl = baseUrl.trimEnd('/')

            val loginPageResponse = client.newCall(
                Request.Builder()
                    .url("$effectiveBaseUrl/default2.aspx")
                    .build()
            ).execute()

            val sessionId = loginPageResponse.headers("Set-Cookie")
            ...
            if (loginResultHtml.contains("验证码") && captcha.isEmpty()) {
                val captchaUrl = "$effectiveBaseUrl/CheckCode.aspx"
                return@withContext SchoolLoginResult(
                    success = false,
                    needCaptcha = true,
            ...
            sessionId?.let { resultCookies["ASP.NET_SessionId"] = it }
    ...
    override suspend fun fetchScheduleHtml(
        ...
        try {
            ...
            val response = client.newCall(
                Request.Builder()
                    .url("$effectiveBaseUrl/xskbcx.aspx?xh=$studentId")
