...
**Server酱通知构建失败时附加关键错误代码**

1. **优化**：构建步骤改为 `tee` 输出到 `build_output.log`，并设置 `continue-on-error: true` 允许后续步骤提取错误
2. **新增**：构建失败时自动提取关键错误行（`e:` / `w:` / `ERROR` / `FAILURE` / `> Task.*FAILED` 开头的行，最多 20 行），无匹配时回退到最后 30 行
3. **优化**：Server酱通知在构建失败时附加错误代码块，使用 Markdown 代码格式展示

### 修改文件

- `.github/workflows/release.yml`（构建步骤 tee 输出 + 提取错误步骤 + 通知附加错误信息）
